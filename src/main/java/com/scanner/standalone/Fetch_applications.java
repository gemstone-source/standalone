...
public class Fetch_applications {
    public  void fetch() {
        try {
            Process process = Runtime.getRuntime().exec("powershell \"Get-ItemProperty HKLM:\\Software\\Wow6432Node\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\* | Select-Object DisplayName,DisplayVersion,InstallDate | Convertto-json |out-file 'apps.json' \"");
            java.util.Scanner scanner = new java.util.Scanner(process.getInputStream()).useDelimiter("\\A");
            String result = scanner.hasNext() ? scanner.next() : "";
            System.out.println(result);

            //remove null values
            File filePath = new File("apps.json");
